...
export default function Home({ liff, liffError }) {
  ...
  return <><Head><title>一起點餐 · Order Together</title><meta name="viewport" content="width=device-width, initial-scale=1" /></Head><nav className="top-tabs" aria-label="Order navigation"><button className={activeTab === "order" ? "top-tab active" : "top-tab"} onClick={() => setActiveTab("order")}>Order</button><span className="top-tab-divider" /><button className={activeTab === "archive" ? "top-tab active" : "top-tab"} onClick={() => setActiveTab("archive")}>Archive</button></nav>{isWorking && <div className="request-buffer" role="status" aria-live="polite"><div><span className="spinner" /><strong>Saving changes</strong><small>Please wait a moment…</small></div></div>}<main className="order-page"><div className="tenant-list">
    ...
    {visibleTenants.map((tenant, index) => <section className="order-card" key={tenant.id}><header className="order-header"><div><p className="eyebrow">{activeTab === "archive" ? "ARCHIVED TENANT" : `TENANT GROUP ${index + 1}`}</p><div className="tenant-title"><h1>{tenant.name}</h1>{tenant.role === "owner" && !tenant.is_archived && <button className="edit-tenant" onClick={() => openTenant(tenant)}>Edit</button>}</div><p className="subtitle">一起接龍點餐，輕鬆總結每一份心意。</p></div><div className="order-count"><strong>{tenant.orders.reduce((total, order) => total + Number(order.quantity), 0)}</strong><span>items</span></div></header>{!tenant.is_archived && <><div className="tenant-tools">{tenant.role === "owner" && <><button className="invite-button" onClick={() => inviteFriends(tenant)}>Invite LINE friends</button><label className="menu-upload"><input type="file" accept="image/*" onChange={(event) => uploadMenu(event, tenant)} />{tenant.menuImageUrl ? "Re-upload menu" : "Upload menu"}</label><button className="finish-button" disabled={!tenant.orders.length} onClick={() => { setActiveTenant(tenant); setModal("archive"); }}>Order Finish</button></>}</div><section className="menu-section"><div><p className="eyebrow">TENANT MENU</p><p>{tenant.menuImageUrl ? "Menu image · compressed WebP" : "Upload one menu image for this tenant."}</p></div>{tenant.menuImageUrl && <img src={tenant.menuImageUrl} alt={`${tenant.name} menu`} />}</section><div className="table-wrap"><table><thead><tr><th>Index</th><th>Menu</th><th>Quantity</th><th>Who order</th><th>Notes</th><th aria-label="Actions" /></tr></thead><tbody>{tenant.orders.length ? tenant.orders.map((order, orderIndex) => <tr key={order.id}><td data-label="Index">{String(orderIndex + 1).padStart(2, "0")}</td><td data-label="Menu" className="menu-name">{order.menu}</td><td data-label="Quantity"><span className="quantity">{order.quantity}</span></td><td data-label="Who order">{order.ordered_by_name}</td><td data-label="Notes" className="notes">{order.notes || "—"}</td><td className="actions"><button onClick={() => openOrder(tenant, order)}>Edit</button><button className="delete-link" onClick={() => { setActiveTenant(tenant); setActiveOrder(order); setModal("delete"); }}>Delete</button></td></tr>) : <tr><td className="empty-orders" colSpan="6">No active orders.</td></tr>}</tbody></table></div><button className="add-button" onClick={() => openOrder(tenant)} disabled={loginState !== "ready"}><span>＋</span>Add order</button></>}{tenant.archives?.length > 0 && <section className="archive-section"><p className="eyebrow">ARCHIVED ORDERS</p>{tenant.archives.map((archive) => <details key={archive.id} open={openArchiveId === archive.id} onToggle={(event) => setOpenArchiveId(event.currentTarget.open ? archive.id : null)}><summary><span>{new Date(archive.archived_at).toLocaleDateString()}</span><span>{archive.total_items} items · Total {formatAmount(archive.total_amount)}</span></summary><ul>{archive.orders.map((order) => <li key={order.id}>{order.quantity}× {order.menu} <span>{order.ordered_by_name}</span></li>)}</ul></details>)}</section>}</section>)}
