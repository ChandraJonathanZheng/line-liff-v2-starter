...
export default function Home({ liff, liffError }) {
  ...
  const togglePaid = async (tenant, order) => {
    setIsWorking(true);
    try { await api("POST", { action: "order.pay", tenantId: tenant.id, orderId: order.id, isPaid: !order.is_paid }); await loadTenants(true); }
    catch (requestError) { setError(requestError.message); } finally { setIsWorking(false); }
  };
  ...
  return <>
    ...
    <main className="order-page"><div className="tenant-list">
      ...
      {visibleTenants.map((tenant, index) => {
        ...
        const paidCount = tenant.orders.filter((order) => order.is_paid).length;
        const allPaid = tenant.orders.length > 0 && paidCount === tenant.orders.length;
        const myOrders = tenant.orders.filter((order) => order.ordered_by_line_user_id === viewerId);
        const myPaid = myOrders.length > 0 && myOrders.every((order) => order.is_paid);
        ...
        return <section className="order-card" key={tenant.id}>
          ...
          <section className="order-summary" aria-label={`${tenant.name} summary`}><div><span>Group total</span><strong>{formatCurrency(groupTotal, tenant.currency_code)}</strong></div><div><span>My order</span><strong>{formatCurrency(myTotal, tenant.currency_code)}</strong></div><div><span>People</span><strong>{new Set(tenant.orders.map((order) => order.ordered_by_line_user_id)).size}</strong></div><div><span>My status</span><strong className={myPaid ? "payment-complete" : ""}>{myOrders.length === 0 ? "No order yet" : myPaid ? "Paid" : "Unpaid"}</strong></div><div><span>Payment</span><strong className={allPaid ? "payment-complete" : ""}>{allPaid ? "All paid" : `${paidCount}/${tenant.orders.length} paid`}</strong></div></section>
