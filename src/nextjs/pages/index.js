...
export default function Home({ liff, liffError }) {
  ...
  return <><Head><title>一起點餐 · Order Together</title><meta name="viewport" content="width=device-width, initial-scale=1" /></Head><nav className="top-tabs" aria-label="Order navigation"><button className={activeTab === "order" ? "top-tab active" : "top-tab"} onClick={() => setActiveTab("order")}>Order</button><button className={activeTab === "archive" ? "top-tab active" : "top-tab"} onClick={() => setActiveTab("archive")}>Archive</button></nav>{isWorking && <div className="request-buffer" role="status" aria-live="polite"><div><span className="spinner" /><strong>Saving changes</strong><small>Please wait a moment…</small></div></div>}<main className="order-page"><div className="tenant-list">
    ...
    {visibleTenants.map((tenant, index) => {
      if (tenant.is_archived) {
        ...
        return <details className="order-card archive-tenant-card" key={tenant.id} open={openArchiveTenantId === tenant.id} onToggle={(event) => setOpenArchiveTenantId(event.currentTarget.open ? tenant.id : null)}>
          <summary className="order-header archive-tenant-summary"><div><p className="eyebrow">ARCHIVED TENANT</p><div className="tenant-title"><h1>{tenant.name}</h1></div><p className="subtitle">{tenant.description || "View archived orders"}</p>{latestArchive && <p className="archive-summary">{latestArchive.total_items} items · Total {formatAmount(latestArchive.total_amount)}</p>}</div><span className="archive-toggle-label">Show</span></summary>
      ...
      return <section className="order-card" key={tenant.id}><header className="order-header"><div><p className="eyebrow">{`TENANT GROUP ${index + 1}`}</p><div className="tenant-title"><h1>{tenant.name}</h1>{tenant.role === "owner" && <button className="edit-tenant" onClick={() => openTenant(tenant)}>Edit</button>}</div><p className="subtitle">{tenant.description || "一起接龍點餐，輕鬆總結每一份心意。"}</p></div><div className="order-count"><strong>{tenant.orders.reduce((total, order) => total + Number(order.quantity), 0)}</strong><span>items</span></div></header><div className="tenant-tools">{tenant.role === "owner" && <><button className="invite-button" onClick={() => inviteFriends(tenant)}>Invite LINE friends</button><label className="menu-upload"><input type="file" accept="image/*" onChange={(event) => uploadMenu(event, tenant)} />{tenant.menuImageUrl ? "Re-upload menu" : "Upload menu"}</label><button className="finish-button" disabled={!tenant.orders.length} onClick={() => { setActiveTenant(tenant); setModal("archive"); }}>Order Finish</button></>}</div><section className="menu-section"><div><p className="eyebrow">TENANT MENU</p><p>{tenant.menuImageUrl ? "Tap the image to view it full size." : "Upload one menu image for this tenant."}</p></div>{tenant.menuImageUrl && <button type="button" className="menu-preview" onClick={() => setMenuPreview({ url: tenant.menuImageUrl, name: tenant.name })} aria-label={`View ${tenant.name} menu`}><img src={tenant.menuImageUrl} alt={`${tenant.name} menu`} /></button>}</section><div className="table-wrap"><table><thead><tr><th>Index</th><th>Menu</th><th>Quantity</th><th>Who order</th><th>Notes</th><th aria-label="Actions" /></tr></thead><tbody>{tenant.orders.length ? tenant.orders.map((order, orderIndex) => <tr key={order.id}><td data-label="Index">{String(orderIndex + 1).padStart(2, "0")}</td><td data-label="Menu" className="menu-name">{order.menu}</td><td data-label="Quantity"><span className="quantity">{order.quantity}</span></td><td data-label="Who order">{order.ordered_by_name}</td><td data-label="Notes" className="notes">{order.notes || "—"}</td><td className="actions"><button onClick={() => openOrder(tenant, order)}>Edit</button><button className="delete-link" onClick={() => { setActiveTenant(tenant); setActiveOrder(order); setModal("delete"); }}>Delete</button></td></tr>) : <tr><td className="empty-orders" colSpan="6">No active orders.</td></tr>}</tbody></table></div><button className="add-button" onClick={() => openOrder(tenant)} disabled={loginState !== "ready"}><span>＋</span>Add order</button>{tenant.role === "owner" && <button className="delete-tenant-button" onClick={() => { setActiveTenant(tenant); setModal("deleteTenant"); }}>Delete Tenant</button>}</section>;
    })}
  ...
  {modal === "deleteTenant" && <div className="modal-backdrop"><section className="modal confirm-modal"><div className="warning-icon">!</div><p className="eyebrow">DELETE TENANT</p><h2>Delete {activeTenant?.name}?</h2><p>This permanently removes the tenant, its orders, and its archived orders.</p><div className="modal-actions"><button className="secondary-button" onClick={() => setModal(null)}>Cancel</button><button className="danger-button" onClick={deleteTenant}>Delete Tenant</button></div></section></div>}
  ...
  {menuPreview && <div className="modal-backdrop menu-preview-backdrop" role="dialog" aria-modal="true" aria-label={`${menuPreview.name} menu preview`} onClick={() => setMenuPreview(null)}><button type="button" className="menu-preview-close" onClick={() => setMenuPreview(null)} aria-label="Close menu preview">×</button><img className="menu-preview-full" src={menuPreview.url} alt={`${menuPreview.name} menu`} onClick={(event) => event.stopPropagation()} /></div>}
  </>;
}
